...
if __name__ == '__main__':
	...
	parser.add_argument('--batch_size',         type=str, required=True,	help='batch size for inference 							[required]')
	parser.add_argument('--model_input_dims',	type=str, required=True,	help='c,h,w - batch size,channel,height,width           [required]')
	parser.add_argument('--model_output_dims',	type=str, required=True,	help='c,h,w - batch size,channel,height,width           [required]')
	...
	modelBatchSize = args.batch_size
	...
	if (not os.path.isfile(analyzerDir + "/setupFile.txt")):
		...
		f.write(modelFormat + ';' + modelName + ';' + modelLocation + ';' + modelBatchSize + ';' + modelInputDims + ';' + modelOutputDims + ';' + label + ';' + outputDir + ';' + imageDir + ';' + imageVal + ';' + hierarchy + ';' + str(Ax).strip('[]').replace(" ","") + ';' + str(Mx).strip('[]').replace(" ","") + ';' + fp16 + ';' + replaceModel)
		f.close()
	else:
		count = len(open(analyzerDir + "/setupFile.txt").readlines())
		if count < 10:
			f = open(analyzerDir + "/setupFile.txt", "a")
			f.write("\n" + modelFormat + ';' + modelName + ';' + modelLocation + ';' + modelBatchSize + ';' + modelInputDims + ';' + modelOutputDims + ';' + label + ';' + outputDir + ';' + imageDir + ';' + imageVal + ';' + hierarchy + ';' + str(Ax).strip('[]').replace(" ","") + ';' + str(Mx).strip('[]').replace(" ","") + ';' + fp16 + ';' + replaceModel)
			f.close()
		else:
	...
	for x,(image_batch, image_tensor) in enumerate(imageIterator,0):
		...
		#for img_ind in range(loader.getOutputImageCount()):
			...
			if augmentedResults[i] == 0:
				cloned_image = image_batch[0:224, (i*224+i):(i*224+i+1)]
				cv2.imshow('cloned_image', cv2.cvtColor(cloned_image, cv2.COLOR_RGB2BGR))
				cv2.rectangle(image_batch[ 0:224, (i*224+i):(i*224+i+1)], ((i*224)+i,0),((i*224)+i+1,224), (255,0,0), 4, cv2.LINE_8, 0)
				cv2.waitKey(0)
				...
				cloned_image = image_batch[(i*224+i):(i*224+i+1)]
				cv2.imshow('cloned_image', cv2.cvtColor(cloned_image, cv2.COLOR_RGB2BGR))
				cv2.rectangle(image_batch[(i*224+i):(i*224+i+1) , 0:224], ((i*224)+i,0),((i*224)+i+1,224), (0,255,0), 4, cv2.LINE_8, 0)
				cv2.waitKey(0)
				#cloned_image[(i*224+i):(i*224+i+1) , 0:224] = image_batch[(i*224+i):(i*224+i+1) , 0:224]
